...
from src.recognise import features, matcher  # noqa: E402
from src.recognise.preprocess_sig import (  # noqa: E402
    centre_by_mass,
    normalise_signature,
    resize_keep_aspect,
    trim_to_ink,
...
def _build_norm_step_images(
    sample: matcher.SignatureSample,
...
def save_normalisation_steps(sample: matcher.SignatureSample, out_path: Path) -> None:
    """Save a six-panel figure showing each normalisation step."""
...
def _good_orb_matches(
    norm_a: np.ndarray, norm_b: np.ndarray
) -> tuple[list[cv2.KeyPoint], list[cv2.KeyPoint], list[cv2.DMatch], np.ndarray | None, np.ndarray | None]:
...
def save_orb_matches(
    left: matcher.SignatureSample, right: matcher.SignatureSample, out_path: Path
) -> None:
    """Save a side-by-side figure of two signatures with ORB match lines drawn."""
...
def save_flagged_example(
    samples: list[matcher.SignatureSample], out_path: Path
) -> None:
    """Save a strip of all samples with the outlier boxed in red."""
    if len(samples) < 2:
        _placeholder_figure(
            "Flagged example", "Need at least two samples to build an outlier figure.", out_path
        )
        return
    ...
    figure, axes = plt.subplots(
        1, len(samples), figsize=(max(6.0, 2.4 * len(samples)), 3.8), squeeze=False
    )
    axes_row = axes[0]
    for position, (axis, sample, mean_score) in enumerate(
        zip(axes_row, samples, means, strict=False)
    ):
        axis.imshow(
            sample.crop if sample.crop is not None else sample.mask,
            cmap=None if sample.crop is not None else "gray",
        )
        axis.set_title(
            f"{matcher.short_sheet_date(sample.sheet_date)}\nmean {mean_score:.2f}", fontsize=9
        )
        axis.axis("off")
...
def build_evaluation(save_only: bool = True) -> EvaluationSummary:
    ...
    _no_crops = not samples_by_student
    if _no_crops:
        for fname, title in [
            ("m8_score_distributions.png", "M8 recognition"),
            ("m8_far_frr_curve.png", "M8 recognition"),
            ("m8_feature_comparison.png", "M8 recognition"),
            ("m8_similarity_matrix.png", "M8 recognition"),
            ("m8_orb_matches.png", "M8 recognition"),
            ("m8_normalisation_steps.png", "M8 recognition"),
            ("m8_flagged_example.png", "M8 recognition"),
        ]:
            _placeholder_figure(
                title,
                "No saved crops were found under outputs/cells/ yet.",
                config.FIGURES / fname,
            )
    ...
    matcher.save_pair_distribution_figure(
        genuine_scores, impostor_scores, config.FIGURES / "m8_score_distributions.png"
    )

    thresholds, fars, frrs, eer_threshold, eer_rate = matcher.threshold_curve(
        genuine_scores, impostor_scores
    )
    matcher.save_threshold_curve_figure(
        thresholds, fars, frrs, eer_threshold, eer_rate,
    ...
    best_orb_pair = max(
        (pair for pair in genuine if pair.score.orb > 0.0),
        key=lambda pair: pair.score.orb,
        default=None,
    )
    if best_orb_pair is not None:
        save_orb_matches(
            best_orb_pair.left, best_orb_pair.right, config.FIGURES / "m8_orb_matches.png"
        )
    else:
        _placeholder_figure(
            "ORB matches",
